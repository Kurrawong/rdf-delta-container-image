...
echo_log "Step 1/4: Cleaning up and creating Fuseki database directory..."
rm -rf fuseki
mkdir -p fuseki/databases
echo_log "Step 1/4: Completed - Database directory ready at ./fuseki/databases"

# Step 2: Load RDF data using Docker container
echo_log "Step 2/4: Loading RDF data with fuseki-lucene-shacl-loader (tdb2.xloader mode)..."
docker run \
        --rm \
        --name loader \
        ...
        --volume "./MAGIX:/rdf" \
        --volume "./fuseki/databases:/fuseki/databases" \
        --env "MODE=tdb2.xloader" \
        --env "NO_VALIDATION=true" \
        "fuseki-lucene-shacl-loader:6.2.0-SNAPSHOT"
echo_log "Step 2/4: Completed - RDF data loaded"

# Step 3: Apply SPARQL update queries
echo_log "Step 3/4: Applying SPARQL update queries..."
echo_log "  - Running magix.rq..."
tdb2.tdbupdate --loc=fuseki/databases/myds --update=queries/magix.rq
echo_log "  - Running magix2.rq..."
tdb2.tdbupdate --loc=fuseki/databases/myds --update=queries/magix2.rq
echo_log "Step 3/4: Completed - SPARQL updates applied"

# Step 4: Build text indexes
echo_log "Step 4/4: Building text indexes with fuseki-lucene-shacl-loader (text mode)..."
docker run \
        --rm \
        --name indexer \
        --user "$USER_UID:$USER_GID" \
        ...
        --env "NO_VALIDATION=true" \
        "fuseki-lucene-shacl-loader:6.2.0-SNAPSHOT"
echo_log "Step 4/4: Completed - Text indexes built"
